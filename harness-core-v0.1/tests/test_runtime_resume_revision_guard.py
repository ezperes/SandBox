...
from harness.core.state.resume_policy import ResumeStatusDecision, ResumeStatusRejected
...
def records():
    return {
        ...
        "CTX-T1": {"revision_ref": "CTX-T-REV-1", "context_ref": "CTX-T1", "estimated_tokens": 10, "required": True},
        "CTX-X1": {"revision_ref": "CTX-X-REV-1", "context_ref": "CTX-X1", "estimated_tokens": 10, "required": True},
        "CTX-N1": {"revision_ref": "CTX-N-REV-1", "context_ref": "CTX-N1", "estimated_tokens": 10, "required": True},
        "TASK": {
...
def test_resume_preparation_read_set_covers_identity_authority_task_and_materialized_context():
    ...
    assert {"ID-A1", "AUT-T", "AUT-X", "AUT-N", "TASK", "CTX-T1", "CTX-X1", "CTX-N1"} <= refs
    materialized_sources = {ref for chain_sources in preparation.context.materialized_source_refs.values() for ref in chain_sources}
...
def test_released_guard_generation_cannot_release_or_replay_over_current_resume_guard():
    source = InMemorySourceAdapter(records())
    old_reads = VersionedReadSet(); read_versioned_for_sensitive_use(source, "AUT-X", old_reads)
    old_guard = source.acquire_revision_guard(old_reads, "RESUME:R1:CP-1")
    source.release_revision_guard(old_guard)
    current_reads = VersionedReadSet(); read_versioned_for_sensitive_use(source, "AUT-X", current_reads)
...
def test_repeated_resume_of_completed_checkpoint_is_rejected_before_second_runtime_call():
    ...
    with pytest.raises(ResumeStatusRejected) as exc:
        manager.resume(make_run(), runtime, checkpoint.checkpoint_id, freshness_gate=second_gate)
    assert exc.value.result.decision == ResumeStatusDecision.BLOCK_TERMINAL
    assert runtime.resume_calls == 1
...
def test_source_without_strong_revision_capability_fails_closed_before_runtime():
    ...
    class WeakSource:
        def read(self, source_ref): return strong_source.read(source_ref)
    gate = ResumeFreshnessGate(
        source=WeakSource(),
        identity_source_ref="ID-A1",
        task_source_ref="TASK",
