...
def records():
    return {
        ...
        "AUT-T": {
            ...
            "allowed_scopes": ["ops:resume", "ops:write"],
        },
        "AUT-X": {
...
def make_run(run_id="R1"):
    return HarnessRun(
        ...
        authority_context_ref=f"AC-{run_id}-OLD",
    )


def checkpointed_state(manager, run_id="R1", *, evidence_refs=None):
    state = RunState(
        run_state_id=f"RS-{run_id}",
        run_id=run_id,
        tarefa_trabalho_id="MT-1",
        status=RunStatus.INTERRUPTED,
        current_step="resume-side-effect",
        completed_steps=["side-effect"],
        pending_steps=["resume-side-effect"],
    )
    manager.persist(state)
    checkpoint = manager.checkpoint(
        state,
        validated_step="side-effect",
        resume_instruction="resume without replaying proved effects",
...
def gateway(manager, source, *, tool_id="ops.write"):
    ...
    adapter = FakeToolAdapter({"ok": True, "evidence_refs": ["EV-SIDE-EFFECT"]})
...
def test_completed_side_effect_checkpoint_resume_does_not_repeat_external_effect():
    ...
    gw, adapter = gateway(manager, source)
    ...
    class ReplayRuntime:
        def resume(self, run, current_state):
            fresh_authority = AuthorityResolver(source).resolve(run.run_id, identity).context
            assert_retry_blocked(lambda: gw.execute(
                run_id=run.run_id,
    ...
    manager.resume(make_run(), ReplayRuntime(), checkpoint.checkpoint_id,
...
def test_repeated_resume_of_same_checkpoint_never_duplicates_completed_effect():
    source = InMemorySourceAdapter(records())
    identity, authority, context = resolve(source)
    manager = StateManager(InMemoryStateAdapter())
    gw, adapter = gateway(manager, source)
    first = gw.execute(run_id="R1", authority=authority.context, tool_id="ops.write",
                       payload={}, business_key="ORDER-REPEAT")
    _, checkpoint = checkpointed_state(manager, evidence_refs=list(first.evidence_refs))
...
def test_failed_retry_requires_explicit_opt_in_and_preserves_attempt_semantics():
    ...
    assert retried.status == IdempotencyStatus.PENDING
    assert retried.attempt == 2
    assert retried.error is None
    assert retried.reconciliation_required is False
...
def test_revision_change_revalidates_resume_but_completed_effect_still_does_not_repeat():
    ...
    gw, adapter = gateway(manager, source)
    first = gw.execute(run_id="R1", authority=authority.context, tool_id="ops.write",
                       payload={}, business_key="ORDER-REVISION")
    _, checkpoint = checkpointed_state(manager, evidence_refs=list(first.evidence_refs))
    ...
    class ReplayAfterRevalidationRuntime:
        def resume(self, run, current_state):
            fresh_authority = AuthorityResolver(source).resolve(run.run_id, identity).context
            assert fresh_authority.tactical_chain_trace.source_revision_refs == ["T-REV-2"]
            assert_retry_blocked(lambda: gw.execute(
                run_id=run.run_id,
                authority=fresh_authority,
                tool_id="ops.write",
                payload={},
                business_key="ORDER-REVISION",
            ))
            current_state.status = RunStatus.COMPLETED
            return current_state

    manager.resume(make_run(), ReplayAfterRevalidationRuntime(), checkpoint.checkpoint_id,
                   freshness_gate=resume_gate(source, identity, authority, context))

    assert len(adapter.calls) == 1
    resume_records = [r for r in port.list_revalidation_records("R1") if r["boundary"] == "RuntimePort.resume"]
    assert len(resume_records) == 1
    assert resume_records[0]["status"] == "RELEASED"
...
def test_cross_run_same_business_key_does_not_reuse_another_runs_ledger():
    source = InMemorySourceAdapter(records())
    _, authority_r1, _ = resolve(source, "R1")
    _, authority_r2, _ = resolve(source, "R2")
    manager = StateManager(InMemoryStateAdapter())
    gw, adapter = gateway(manager, source)

    r1 = gw.execute(run_id="R1", authority=authority_r1.context, tool_id="ops.write",
                    payload={"run": 1}, business_key="SHARED-ORDER")
    r2 = gw.execute(run_id="R2", authority=authority_r2.context, tool_id="ops.write",
                    payload={"run": 2}, business_key="SHARED-ORDER")
...
def test_same_business_key_cannot_bypass_checkpoint_run_binding_on_wrong_resume():
    ...
    manager = StateManager(InMemoryStateAdapter())
    gw, adapter = gateway(manager, source)
    gw.execute(run_id="R1", authority=authority.context, tool_id="ops.write",
               payload={}, business_key="BOUND-ORDER")
    _, checkpoint = checkpointed_state(manager)
    ...
    runtime = MustNotRun()
    with pytest.raises(HarnessResolutionError) as exc:
        manager.resume(make_run("R2"), runtime, checkpoint.checkpoint_id,
                       freshness_gate=resume_gate(source, identity, authority, context))
...
def test_runtime_resume_failure_preserves_completed_effect_and_revalidation_history():
    ...
    gw, adapter = gateway(manager, source)
    first = gw.execute(run_id="R1", authority=authority.context, tool_id="ops.write",
                       payload={}, business_key="ORDER-RUNTIME-FAIL")
    _, checkpoint = checkpointed_state(manager, evidence_refs=list(first.evidence_refs))

    class FailingRuntime:
        def resume(self, run, current_state):
            raise RuntimeError("runtime crashed after revalidation")

    with pytest.raises(RuntimeError, match="runtime crashed after revalidation"):
        manager.resume(make_run(), FailingRuntime(), checkpoint.checkpoint_id,
                       freshness_gate=resume_gate(source, identity, authority, context))

    ledger = manager.get_side_effect(first.idempotency_key)
    assert ledger.status == IdempotencyStatus.COMPLETED
    assert ledger.evidence_refs == ["EV-SIDE-EFFECT"]
    assert len(adapter.calls) == 1
    assert port.load_checkpoint(checkpoint.checkpoint_id).evidence_refs == ["EV-SIDE-EFFECT"]

    resume_records = [r for r in port.list_revalidation_records("R1") if r["boundary"] == "RuntimePort.resume"]
    assert len(resume_records) == 1
    audit = resume_records[0]
    assert audit["status"] == "FAILED"
    assert audit["outcome"] == "RUNTIME_RESUME_ERROR"
    assert [event["status"] for event in audit["events"]] == ["PENDING", "RELEASED", "FAILED"]
    assert audit["authority_snapshot"]
    assert audit["task_context"]
...
def test_temporal_order_of_tool_and_resume_audit_records_remains_traceable():
    ...
    port = InMemoryStateAdapter()
    manager = StateManager(port)
    gw, _ = gateway(manager, source)
    gw.execute(run_id="R1", authority=authority.context, tool_id="ops.write",
               payload={}, business_key="ORDER-TIME")
    _, checkpoint = checkpointed_state(manager)
    ...
    manager.resume(make_run(), CompleteRuntime(), checkpoint.checkpoint_id,
                   freshness_gate=resume_gate(source, identity, authority, context))
