...
class Goal:
    value = "alea jacta est"
    charset = string.ascii_lowercase+" "
    size = len(value)


class PopSettings:
    pop_init_size = 200         # Population initial size
...
class GeneticSettings:
    population_grow_rate = 0.05         # How much pop grows through generations
    offspring_method_params = {
        'elite_rate': 0.5,       # Ratio from pop considered best fit for random choice
...
def f(x, goal=Goal.value):
    ...
    maxpoints = len(goal)
    x_size = len(x)
    # print("(x_size: %s, maxpoints: %s)" % (x_size, maxpoints), end='   ...   ')

    if maxpoints == x_size:
        # Goal and tested obj lenghts match
...
def x_elite_breeder(pop: pd.DataFrame) -> pd.DataFrame:
    ...
    breed['Fitness'] = breed.Individual.apply(f)

    return breed

def mutation_indv(indv):
    ind = list(indv)
...
def mutation_pop(pop):
    return pop.Individual.apply(mutation_indv)
...
def print_pop_stats(pop: pd.DataFrame, generation: int = None):
    ...
    pprint(newpop.sort_values('Fitness').head(5))
    pass
...
dfpop['Fitness'] = dfpop.Individual.apply(f)
...
while i<GeneticSettings.max_iter and dfpop.Fitness.min()>GeneticSettings.precision_goal:
    ...
    # Appends breed into population
    newpop = dfpop.append(x_breed, ignore_index=True)

    # Mutation phenomena
    mutant = mutation_pop(newpop)
    ...
    newpop['Fitness'] = newpop.Individual.apply(f)
